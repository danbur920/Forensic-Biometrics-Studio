...
const createOverviewCalloutImage = async (
    ...
    placementOrder.forEach(({ feature, angle: slotAngle }) => {
        ...
        let lx = centerX + Math.cos(slotAngle) * baseLabelRadiusX;
        let ly = centerY + Math.sin(slotAngle) * baseLabelRadiusY;
        let placed = false;
        for (let ring = 0; ring < 12 && !placed; ring += 1) {
            const radialBoost = ring * (numberCircleRadius + 6);
            // eslint-disable-next-line no-loop-func
            angularOffsets.some(angleOffset => {
                const angle = slotAngle + angleOffset;
                const candidateX =
                    centerX +
                    Math.cos(angle) * (baseLabelRadiusX + radialBoost);
                const candidateY =
                    centerY +
                    Math.sin(angle) * (baseLabelRadiusY + radialBoost);
                const safeX = clamp(
                    candidateX,
        ...
        ctx.fillStyle = "#1a56db";
        ctx.fillText(String(feature.label), lx, ly + 0.5);
    });
...
const createStyles = () => {
    ...
    style.textContent = `
        ...
        .feature-index {
            ...
            background: #ffffff; color: var(--marker-text, #0c2d80);
            border: 2px solid var(--marker-ring, #1a56db);
            box-shadow: 0 0 0 1px var(--marker-outline, #0c2d80);
            display: inline-flex; align-items: center; justify-content: center;
            font-size: 12px; line-height: 1; font-weight: 700;
            font-family: Arial, sans-serif; transform: translateY(2px);
        }
        .feature-index-value { display: inline-block; line-height: 1; transform: translateY(-7px); }
...
export const generateShoeprintReportPdfWithDialog = async (
    ...
    try {
        ...
        page1.innerHTML = `
        <div class="report-title">${tReport("Shoeprint report title")}</div>
        <div class="meta-grid">
        ...
        <div class="counts">
            <div>${tReport("Shoeprint paired features count")} ${paired.length}</div>
        </div>
        ...
        paired.forEach((feature, idx) => {
            ...
            <td>
                <div class="feature-cell">
                    <div class="feature-index" style="--marker-ring: ${markerRing}; --marker-outline: ${markerOutline}; --marker-text: ${markerOutline};">
                        <span class="feature-index-value">${feature.left.label}</span>
                    </div>
                    <div class="feature-type">${tReport("Feature type")} ${featureType}</div>
                </div>
            </td>
            <td><img class="feature-image" src="${detailCrop.left}" /></td>
            <td><img class="feature-image" src="${detailCrop.right}" /></td>
        `;
